...
#include "../../include/util/NonCopy.h"
#include <string>
#include <memory>
#include <functional>
using namespace std;
...
public:
    ...
    void setConnCallBack(TcpConnCallBack cbFunc) {
        _connFunc = cbFunc;
    }
    void setMsgCallBack(TcpConnCallBack cbFunc) {
        _msgFunc = cbFunc;
    }
    void setCloseCallBackb(TcpConnCallBack cbFunc) {
        _closeFunc = cbFunc;
    }

    // handle任务是返回当前的连接对象的shared_ptr
    void doHandlCallBack(TcpConnCallBack &cbFunc);
    void handleConnCallBack() {
        doHandlCallBack(_connFunc);
    }
    void handleMsgCallBack() {
        doHandlCallBack(_msgFunc);
    }
    void handleCloseCallBack() {
        doHandlCallBack(_closeFunc);
    }
